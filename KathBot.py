...
async def help(ctx):
    ...
    embed.add_field(
        name="info",
        value="Run this command to get a little more info on me!",
        inline=False,
    )
    embed.add_field(
...
async def info(ctx):
    ...
    embed.add_field(
        name="Twitter",
        value="You can visit the dev twitter [here](https://twitter.com/kathbot_dev)!",
        inline=False,
    )
    embed.add_field(
        name="About KathBot!",
...
async def rate(ctx, *args):
    ...
    if (
        ...
        await ctx.send(
            "Hm... I rate {0} a... {1}/10! (ALL people who are rated are automatically an 11/10)".format(
                arg, randint(0, 10)
            )
        )
