...
def debug_error(ctx, error):
    ...
    while os.path.isfile("{0}/Data/ErrorLogs/{1}.txt".format(cwd, errorID)):
        print(
            "An error log with the same ID has already been created. Generting new ID..."
        )
        errorID += 1
...
async def on_ready():
    ...
    await client.change_presence(
        ...
        activity=discord.Game(name="k!help | inside %s servers!" % len(client.guilds)),
    )
...
async def errorcheck(usage, ctx, error):
    ...
    if (
        ...
        embed = discord.Embed(
            title="An error has occurred.", description=" ", color=0xFF88FF
        )
        embed.add_field(
            name="** ** \nPermission error.",
        ...
        embed.add_field(
            name="** ** \nPlease read above message, or contact my developers if you believe this may be a bug.",
            value="Error ID: ``{0}`` \n[KathBot Support Server]({1})".format(
                errorID, inviteLink
            ),
            inline=False,
        )
        ...
        isinstance(error, ArgumentParsingError)
        or isinstance(error, UserInputError)
        ...
        isinstance(error, BotMissingRole)
        or isinstance(error, BotMissingPermissions)
        or isinstance(error, Forbidden)
    ):
        embed = discord.Embed(
            title="An error has occurred.", description=" ", color=0xFF88FF
        )
        embed.add_field(
            name="** ** \nPermission error.",
            value="I do not have the permission to cary out this command.",
            inline=False,
        )
        embed.add_field(
            name="** ** \nPlease read above message, or contact my developers if you believe this may be a bug.",
        ...
        embed = discord.Embed(
            title="An internal error has occurred.", description=" ", color=0xFF88FF
        )
        embed.add_field(
        ...
        embed = discord.Embed(
            title="An i͝m̡p̧͜o̧̕s҉̕͠s̵͝͝i͏̶̨͜b̷̛̕͠l̀͘͞͝ȩ̶́͘͞ error has occurred.",
            description="How the hell are you even seeing this?\n This shouldn't be happening.",
            color=0xFF88FF,
        )
        embed.add_field(
...
async def on_message(message):
    ...
    ):  # This is to exclude bot messages from running commands, to prevent errors with PluralKit and spam with other bots.
...
async def on_guild_join(guild):
    ...
    embed = discord.Embed(
        title="Thanks for adding me to your server!", description=" ", color=0xFF88FF
    )
    embed.add_field(
        name="This bot may send announcements by it's developers to warn of possible downtimes, issues or just simple updates on development!",
        value="If you do not want to see these, please create or keep the kathbot-announcements channel in a hidden area. \nUse k!help for commands!",
        inline=False,
    )
    embed.add_field(
    ...
    try:
        ...
        botChannel = await guild.create_text_channel(
            name="kathbot-announcements",
            overwrites={
        ...
        embed.add_field(
            name="You are seeing this channel because KathBot needs to have somewhere it is capable of sending messages to.",
            value="This is only a temp setup.",
            inline=False,
        )
        ...
        settings = {"Ignore": ["N/A"], "ModRole": True, "Messages": botChannel}
        ...
        await guild.fetch_channel()
        settingsW = open("{0}/Data/GuildSettings/{1}.json".format(cwd, guild.id), "w+")
        settings = {"Ignore": ["N/A"], "ModRoles": True, "Messages": "N/A"}
...
async def eightball(ctx, *args):
    ...
    await message.edit(
        content='In response to {0}, it reads... "{1}"'.format(arg, choice(response))
    )


@client.command(
    name="cat"
)  # I need to make a pylint scraper to get cat images off of a site with strictly cat images for more of a variety.
async def cat(ctx):
    path = "%s/Cats" % (cwd)
...
async def help(ctx):
    ...
    embed = discord.Embed(
        title="KathBot Commands",
        description="Here you can get a full list of this bot's commands!",
        color=0xFF88FF,
    )
    ...
    embed.add_field(
        name="cat",
        value="Sends a cat picture from a selection of 20 cats.",
        inline=False,
    )
    embed.add_field(name="invite", value="Sends a bot invite link.", inline=False)
    embed.add_field(
        name="ping", value="Responds with the bot's current latency.", inline=False
    )
    embed.add_field(
        name="quote [delete|list|store] [str]",
        value="Stores up to 15 quotes!",
        inline=False,
    )
    embed.add_field(
        name="rate [str]",
        value="Rates an argument from a 0 to 10. All people who rate themselves are an 11/10.",
        inline=False,
    )
    embed.add_field(
        name="say [str]",
        value="Makes the bot say anything you want it to.",
        inline=False,
    )
    embed.add_field(
        name="source",
        value="Run this command to get a link to my source code!",
        inline=False,
    )
    embed.add_field(
        name="tarot [int] [optional str]",
        value="Generates a spread of tarot cards, anywhere from between 1 to 7. [WIP]",
        inline=False,
    )
    embed.add_field(
        name="Need support with the bot, have concerns, or have a bug to report?",
...
async def invite(ctx):
    ...
    embed = discord.Embed(
        title="KathBot Invite",
        description="You want to invite me to your server?!",
        color=0xFF88FF,
    )
    embed.set_thumbnail(url="https://puu.sh/Fvnef.png")
    embed.add_field(
        name="Which version would you like to invite?",
        value="[Public](https://discordapp.com/oauth2/authorize?client_id=596683881575612429&scope=bot&permissions=335890512) \n[Public Dev](https://discord.com/oauth2/authorize?client_id=610044394854416404&scope=bot&permissions=335890512)",
    )
    embed.add_field(
        name="Need support with the bot, have concerns, or have a bug to report?",
        value="**Join the [KathBot Support Server!](%s)**" % inviteLink,
...
async def quote(ctx, scmd, *, args=""):
    if scmd == "store" or scmd == "s":
        ...
        if arg == "" or arg == "** **" or arg == " ":
            errorID = debug_error(
                ctx, "User input an invalid or empty quote for storage."
            )
            embed = discord.Embed(
                title="An error has occurred.", description=" ", color=0xFF88FF
            )
        ...
        if len(arg) < 100:
            if os.path.isfile(
                "{0}/Data/QuotesStorage/{1}.json".format(cwd, ctx.author.id)
            ):
                quotesR = open(
                    "{0}/Data/QuotesStorage/{1}.json".format(cwd, ctx.author.id), "r+"
                )
                try:
                ...
                if len(quotesList["quotes"]) > 16:
                    await ctx.send(
                        "Can not store quote. (Quote storage limit exceeded)"
                    )
                    return 0
                ...
                quotesW = open(
                    "{0}/Data/QuotesStorage/{1}.json".format(cwd, ctx.author.id), "w+"
                )
                quotesList = {"quotes": ["persist", arg]}
        ...
        if os.path.isfile("{0}/Data/QuotesStorage/{1}.json".format(cwd, ctx.author.id)):
            try:
                ...
                if len(quotes) <= 1:
                    await ctx.send(
                        "You have not stored any quotes. Store some with ``k!quote store [quote]``!"
                    )
                    return 0

                embed = discord.Embed(
                    title="Here are your stored quotes:",
                    description=" ",
                    color=0xFF88FF,
                )
                for index in quotes:
        ...
        scmd == "delete" or scmd == "d" or scmd == "del"
    ):  # May need to be refined a little bit in the future
        if os.path.isfile("{0}/Data/QuotesStorage/{1}.json".format(cwd, ctx.author.id)):
            quotesR = open(
                "{0}/Data/QuotesStorage/{1}.json".format(cwd, ctx.author.id), "r+"
            )
            ...
            embed = discord.Embed(
                title="Which quote would you like to delete?",
                description='Respond with "cancel" to stop. \nCommand will time out in 30 seconds',
                color=0xFF88FF,
            )
            count = 0
            for i in quotesList:
                    ...
                    if msg.author != ctx.author:
                        ...
                        try:
                            ...
                            try:
                                ...
                                await ctx.send(
                                    '"{0}" has been deleted.'.format(deletedQuote)
                                )
                                break

                            except:
                                await ctx.send(
                                    '"{0}" is not a valid number. Please specify the number above the quote you want deleted.'.format(
                                        msg.content
                                    )
                                )
...
async def rate(ctx, *args):
    ...
    if (
        ...
        or lowerArg == "fir"
        or lowerArg == "drukon"
        or lowerArg == "abyssal void"
        or lowerArg == "abyssalvoid"
        or lowerArg == "abyssal_void"
        or lowerArg == "alexis"
        or lowerArg == "me"
        or lowerArg == "you"
        or lowerArg == "as"
        or lowerArg == "anarch1st_s0uls"
        or lowerArg == "anarch1st s0uls"
        or lowerArg == "anarchist souls"
        or lowerArg == "anarch1sts0uls"
        or lowerArg == "anarchistsouls"
        or lowerArg == "anarchist_souls"
        or lowerArg == "someone"
        or lowerArg == "myria"
    ):
...
async def info(ctx):
    ...
    embed.add_field(
        name="Source Code",
        value="You can find a link to my source code [here](https://github.com/HBS-System/KathBot)! \n** **",
        inline=False,
    )
    embed.add_field(
        name="Trello",
        value="You can visit my trello [here](https://trello.com/b/1VH1OE4n/kathbot-python)! \n** **",
        inline=False,
    )
    ...
    if count in range(1, 8):
        ...
        for card in range(0, count):
            ...
            if flip == 0:
                embed.add_field(
                    name="Card #%s" % card,
                    value=random.choice(cards["cards"]),
                    inline=False,
                )
...
async def mod_settings(ctx, cmd="help", arg="", bool=""):
    if os.path.isfile(
        "{0}/Data/GuildSettings/{1}.json".format(cwd, ctx.guild.id)
    ):  # Checks if invoker has permission to run this command.
        settingsR = open(
    ...
    if cmd == "help":  # This is the help subcommand.
        embed = discord.Embed(
            title="KathBot Settings",
            description="This is a list of all available settings.",
            color=0xFF88FF,
        )
        embed.set_thumbnail(url="https://puu.sh/Fvnef.png")
        embed.add_field(
            name="ignore [#channel] [True/False]",
            value="This command will disable commands in the given channel(s).",
            inline=False,
        )
        embed.add_field(name="list", value="Lists the current settings.", inline=False)
        embed.add_field(
            name="messages [#channel] [True/False]",
            value="This setting changes where announcements go. By default it is set to (channel)",
            inline=False,
        )
        embed.add_field(
            name="mod [role] [add/del]",
            value="This setting changes what roles can access moderation commands. This is set to any role with the Administrator permission ticked by default.",
            inline=False,
        )
        embed.add_field(
            name="regen",
            value="**This command will RESET all of the current settings to default.**",
            inline=False,
        )
        embed.add_field(
            name="Need support with the bot, have concerns, or have a bug to report?",
        ...
        cmd == "ignore"
    ):  # This subcommand will set which channels the bot will ignore commands in.
        pass

    elif cmd == "list":  # This subcommand will list all settings that are in place.
        if os.path.isfile("{0}/Data/GuildSettings/{1}.json".format(cwd, ctx.guild.id)):
            ...
            embed.add_field(
                name="** ** \nA message.",
                value="This command is currently a work in progress.",
                inline=False,
            )
            embed.add_field(
                name="Need support with the bot, have concerns, or have a bug to report?",
            ...
            embed.add_field(
                name="** ** \nSettings error.",
                value="You do not have any settings generated.",
                inline=False,
            )
            embed.add_field(
                name="** ** \nPlease read above message, or contact my developers if you believe this may be a bug.",
                value="Error: Generate a settings file. \n[KathBot Support Server]({1})".format(
                    inviteLink
                ),
                inline=False,
            )
        ...
        cmd == "reset"
    ):  # This subcommand RESETS the settings for the server it has been executed in, if dome properly.
        await ctx.send(
            "Say ``RESET`` if you want to reset. Otherwise, say ``CANCEL`` to cancel reset."
        )
        try:
            while True:
                msg = await client.wait_for("message", check=check, timeout=30.0)
                if msg.author != ctx.author:
                    ...
                    try:
                        ...
                        if strMsg == "CANCEL":
                            ...
                            try:
                                settingsW = open(
                                    "{0}/Data/GuildSettings/{1}.json".format(
                                        cwd, ctx.guild.id
                                    ),
                                    "w+",
                                )
    ...
    if ctx.author.id == 456282270974607361:
        await client.change_presence(
            ...
            activity=discord.Game(
                name="k!help | {0} | inside {1} servers!".format(
                    arg, len(client.guilds)
                )
            ),
        )
        await ctx.send(
            "Status has been changed to ``k!help | {0} | inside {1} servers!``".format(
                arg, len(client.guilds)
            )
        )
...
async def owner_announce(ctx, *, arg):  # WIP
    if ctx.author.id == 456282270974607361:
        for guild in client.guilds:
            embed = discord.Embed(
                title="Announcement",
                description="Announcement from KathBot's Developer(s)",
                color=0xFF88FF,
            )
